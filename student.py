"""
student.py — Lightweight Student model for on-device smartwatch deployment.

Architecture
------------
Input (B, 3, seq_len)   ← 3 wrist modalities: BVP, EDA, TEMP
  │
  DepthwiseSeparable1D(3  → 64)  → SE(64)  → MaxPool
  DepthwiseSeparable1D(64 → 128) → SE(128) → MaxPool
  DepthwiseSeparable1D(128→ 256) → SE(256) → MaxPool
  │
  AdaptiveAvgPool1d(1) → FC → logits (B, num_classes)

The forward pass returns ``(logits, se_weights_list)`` where
``se_weights_list`` is a list of channel-attention vectors from each SE
block — used for feature-based KD alignment with Teacher attention maps.

In_channels for the first block is now 3 (wrist_bvp, wrist_eda, wrist_temp)
instead of the old 2 (chest_ecg, chest_eda).
"""
...
class SEBlock1D(nn.Module):
    ...
    The SE attention weights are extracted and aligned with the Teacher's
    Transformer attention summary during feature-based KD.
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Returns:
            scaled: shape ``(B, C, L)``
            se_weights: shape ``(B, C)``  ∈ [0, 1]
        """
        b, c, _ = x.size()
        se = self.squeeze(x).view(b, c)    # (B, C)
        se = self.excitation(se)            # (B, C)
        scaled = x * se.unsqueeze(-1)       # (B, C, L)
        return scaled, se
...
class StudentModel(nn.Module):
    """Lightweight DS-CNN + SE student for on-device smartwatch (3-modal).

    Accepts ``(B, 3, seq_len)`` — wrist BVP, EDA, TEMP — and outputs:
        - ``logits``:    ``(B, num_classes)``
        - ``se_weights``: list of ``(B, C_i)`` tensors from each SE block

    Robust to missing modalities (any channel zeroed out; SE re-weights the
    remaining channels automatically).
    """
    ...
    def __init__(self, cfg: CFG | None = None) -> None:
        ...
        channels = cfg.student_channels  # e.g. [64, 128, 256]

        # First block in_channels = number of student modalities (3)
        n_student_modalities = len(cfg.student_modalities)  # 3

        self.blocks = nn.ModuleList()
        in_ch = n_student_modalities  # 3 ← wrist_bvp, wrist_eda, wrist_temp
        for ch in channels:
            self.blocks.append(
    ...
    def forward(
        ...
        Returns:
            logits:     ``(B, num_classes)``
            se_weights: list of tensors ``(B, C_i)`` from each SE block
        """
        ...
        pooled = self.gap(out).squeeze(-1)   # (B, C_last)
...
if __name__ == "__main__":
    ...
    dummy = torch.randn(2, 3, cfg.seq_len)   # (B=2, 3 wrist channels, 7680)
    logits, se_list = model(dummy)
    print(f"logits  : {logits.shape}")        # (2, 2)
    for i, se in enumerate(se_list):
        print(f"SE[{i}]   : {se.shape}")      # (2, 64), (2, 128), (2, 256)
    total = sum(p.numel() for p in model.parameters())
    print(f"Student params: {total:,}")

    # Test missing modality — zero out TEMP (channel 2)
    dummy_missing = dummy.clone()
    dummy_missing[:, 2, :] = 0.0
    logits_m, _ = model(dummy_missing)
    print(f"Missing-mod logits: {logits_m.shape}")  # Should still be (2, 2)
